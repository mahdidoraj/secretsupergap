...
function checkLockState( msg, data )
 if ( data[tostring(msg.to.id)]["settings"]["lock_sticker"] == "yes" ) then
  return true;
 elseif ( data[tostring(msg.to.id)]["settings"]["lock_sticker"] == "no" ) then
  return false;
 else 
  return nil 
 end
...
function UnLockStickers(msg, data)
 if ( not checkper(msg) ) then
  return; end
 if ( data[tostring(msg.to.id)]["settings"]["lock_sticker"] == "no" ) then
...
function getLockState(msg, data)
 ...
 elseif ( data[tostring(msg.to.id)]["settings"]["lock_sticker"] == "no" ) then
  ...
  return "Error please contact @PunisheR_OfficiaL ." 
 end
end
-- little about method :D 
function about()
 return "a plugin made by @PunisheR_OfficiaL"
end
...
function on_msg_receive( msg )
 ...
 if ( checkMedia(msg) and checkLockState(msg, data) ) then
  if ( not checkper(msg) ) then
   ...
   send_large_msg ( R , "Sticker & Photo is not allowed here!!" );
